...
function SearchMobile({ onToggleSearch, openSearch }: Props) {
  ...
  const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const isProductsPage =
      ...
      matchPath("/products/:slug", location.pathname) !== null;
    ...
    if (isProductsPage) {
      ...
      target = `/products/all?q=${encodeURIComponent(query)}`;
    }
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        {focused && search && (
          <div className="fixed left-1/2 translate-x-[-50%] z-12 w-full bg-white shadow-lg border-gray-300 border"></div>
        )}
      </div>
    </div>
  );
