...
function Home() {
  const { products: products1 } = useGetActiveProducts(12);
  const { products: products2 } = useGetBestsellerProducts(12);
    ...
    <section className="w-full">
      <div className="max-w-[1200px] mx-auto flex gap-[15px]">
        ...
        <main className="flex-1 min-w-0 bg-white">
          <MainBanner />
          <ProductSlider products={products1} title="Mới nhất" />
          <ProductSlider products={products2} title="Bán chạy nhất" />
          <PromotionBanner
            banners={[
              {
